...
public class Fox extends Species{
	...
	public Fox(){
		...
		sinceFed = 0;
		isPregnant = false;
		pregTimer= 30; //in days //MAY NEED TWEAKING
		litterSize= 4;
		vision = 5;
		lifespan = 2000;  //MATURES AT 1875
		birthRadius = 2;
	...
	// Feed(){
	// LookforFood()
	// if LookforFood returns true
	//  move() to closest food
	//  clear/unpaint the obecjt?
	// Schedule Idle()
	// Schedule Feed() tomorrow

	
    public Event selector(int a, int b, int action, Species[][] array, EventQueue[] queue, int i){
    	System.out.println("hit fox selector");
    ...
    public Event Move(int a, int b, Species[][] array, EventQueue[] queue, boolean targeting){
    	System.out.println("hit fox move");
    ...
    public Event moveTo(int a, int b, int c, int d, Species[][] array, EventQueue[] queue){
    	System.out.println("hit fox moveto");
            int oldx = a;
            int oldy = b;
            int dx = c;
            int dy = d;
...
//		queue = ev2.queue;
//		array = ev2.array;
    	System.out.println("hit fox idle");
    ...
    public Event asleep(int a, int b, Species[][] array, EventQueue[] queue, int i) //state 5
    {
    	...
    	System.out.println("hit fox asleep");
    	if (array[a][b].isAlive == false){
        	array[a][b] = new Species();
        	Event event= new Event(queue, array);
        	return event;
    ...
    public Event Feed(int a, int b, Species[][] array, EventQueue[] queue, int i){ //state 1
    	System.out.println("hit fox feed");
    	if (array[a][b].isAlive == false){
        	array[a][b] = new Species();
        	Event event= new Event(queue, array);
        	return event;
            ...
            if (array[a][b].rest <= 0){
                ...
                if (dx+dy == a+b){
                	...
                	array[target.x][target.y].isAlive = false;
    ...
    public Event Mate(int a, int b, Species[][] array, EventQueue[] queue, int i){ //state 1
    	System.out.println("hit fox mate");
    	if (array[a][b].isAlive == false){
        	array[a][b] = new Species();
        	Event event= new Event(queue, array);
        	return event;
    ...
    public boolean checker2(int a, int b, Species[][] array){
    	if(a>0 && a<240 && b>0 && b<240){
    		//System.out.println("checking");
    		if(array[a][b].color == Color.red.brighter()){
    			return true;
    		}
    	}
    	return false;
    ...
    public Point isNearFood(int a, int b, Species[][] array){
    	System.out.println("hit fox isnearfood");
    ...
    public Point isNearMate(int a, int b, Species[][] array){
    	System.out.println("hit fox isnearmate");
        int radius = array[a][b].vision;
    	int counter=0;
    	int x;
    	int y;
    ...
    public Event Sleep(int a, int b, Species[][] array, EventQueue[] queue, int i){//state 4
    	System.out.println("hit fox sleep");
   ...
   public Event Reproduce(int a, int b, Species[][] array, EventQueue[] queue ){ 
	   System.out.println("hit fox reproduce");
		...
		while(i<=litterSize-1){
	        ...
	        x = a+x;
	        y=b+y; 
	        System.out.println("new fox attempt at ("+ (x-120) +","+ (y-120)+")");
	        if((x-120)*(x-120) + (y-120)*(y-120) < 14400 ){
	        	if(x > 1 && x < 239 && y > 1 && y < 239){
